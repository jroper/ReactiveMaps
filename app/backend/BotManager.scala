...
object BotManager {
  def props(regionManagerClient: ActorRef, userDistanceRegister: ActorRef, data: Seq[URL]): Props =
    Props(classOf[BotManager], regionManagerClient, userDistanceRegister, data)
 ...
 */
class BotManager(regionManagerClient: ActorRef, userDistanceRegister: ActorRef, data: Seq[URL]) extends Actor {
  import BotManager._
            ...
            { invalid =>
              ...
              feature._1.geometry match {
                case route: LineString[LatLng] if total < max =>
                  ...
                  context.actorOf(GeoJsonBot.props(route, offset, userId, regionManagerClient, userDistanceRegister))
                case other =>
              }
            })
      }
